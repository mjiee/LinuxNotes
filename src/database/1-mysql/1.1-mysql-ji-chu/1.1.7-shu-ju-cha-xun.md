...
where_condition: (查询条件)
  ...
  score is null | is not null  // 为空条件
...
where_ref: (子查询关系, 是可嵌套的)
  ...
  exists (select * from tb_B where ...);  // 存在
  not exists (select * from tb_B ...);  // 不存在
*/
```
...
```sql
# 关联查询
> select * from a [inner] join b on a.c_id = b.c_id where ...;  /* 内连, a & b */ 
> select * from a left join b on a.c_id = b.c_id where ...;  /* 左外连接, a */ 
> select * from a left join b on a.c_id = b.c_id where b.c_id is null;  /* 左外连接, ^b */
> select * from a right join b on a.c_id = b.c_id where ...;  /* 右外连接, b */
...
# 多查询结果合并
> select ... union [all] select ...;  /* 默认是去重复的, all不去重 */
