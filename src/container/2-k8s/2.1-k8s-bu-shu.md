...
# --nodes=2  # 指定节点数
# --network-plugin=cni --extra-config=kubeadm.pod-network-cidr=192.168.0.0/16  # 网络类型和网段
# --extra-config=apiserver.service-node-port-range=1-65535  # 开放本地的网络端口用户nodeport模式 
# --image-mirror-country=cn --image-repository='registry.cn-hangzhou.aliyuncs.com/google_containers' # 指定镜像

# 部署calico
$ curl -O https://projectcalico.docs.tigera.io/manifests/calico.yaml
$ kubectl apply -f calico.yaml  # 部署前去掉CALICO_IPV4POOL_CIDR注释, 设置值为: 192.168.0.0/16
